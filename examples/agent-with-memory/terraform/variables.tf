# Variables for Memory Agent deployment
...
variable "memory_name" {
  description = "Name for the AgentCore memory. Defaults to agent_name."
  type        = string
  default     = null
}
...
variable "memory_description" {
  ...
  type        = string
  default     = "Persistent memory for agent conversation context"
}

variable "memory_event_expiry_duration" {
  description = "Days after which memory events expire (7-365)."
  type        = number
  default     = 30
}

variable "create_memory_kms_key" {
  description = "Create a dedicated KMS key for memory encryption."
  type        = bool
  default     = false
}
